  ...
  return (
    <div>
      <h2 id="car-heading" data-cy="CarHeading">
        <Translate contentKey="carRepairServiceApp.car.home.title">Cars</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="carRepairServiceApp.car.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/car/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="carRepairServiceApp.car.home.createLabel">Create new Car</Translate>
          </Link>
        </div>
      </h2>
        ...
        {carList && carList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="carRepairServiceApp.car.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('manufacturedYear')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('manufacturedYear')} />
                </th>
            ...
            <tbody>
              {carList.map((car, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{car.ownerName}</td>
                  <td>
                    {car.carRepairAppointment ? (
                      <Link to={`/car-repair-appointment/${car.carRepairAppointment.id}`}>{car.carRepairAppointment.date}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="carRepairServiceApp.car.home.notFound">No Cars found</Translate>
            </div>
          )
        )}
      </div>
